...
use App\Models\Brigadista\SubBrigadista;
use App\Models\Brigadista\RelSubbrigadistaMes;
use Carbon\Carbon;

class SubProcesoBrigadistaController extends Controller
{
    public function index(Request $request)
    {
        try{
            ...
            $fecha = Carbon::now();
            $obj = SubBrigadista::with(['brigadista','mes'=>function($query) use ($fecha){
            ...
            /*$totales = BrigadistaMes::whereNull("deleted_at")
    ...
    public function show(Request $request, $id)
    {
        try{
            $parametros = $request->all();
            $fecha = Carbon::now();
            $obj = SubBrigadista::with('mes')->find($id);
    ...
    public function store(Request $request)
    {
        ...
        try {
                ...
                $object->brigadista_id  = $inputs['brigadista_id'];
                $object->save();

                for ($i=1; $i <=12 ; $i++) { 
                    $object2 = new RelSubbrigadistaMes();
                    $object2->subbrigadista_id = $object->id;
                    $object2->mes           = $i;
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            DB::beginTransaction();
            $object = SubBrigadista::find($id);
            $object->descripcion    = $inputs['descripcion'];
            $object->save();

            for ($i=1; $i <=12 ; $i++) { 
                $object2 = RelSubbrigadistaMes::where("subbrigadista_id", $id)->where("mes", $i)->first();
                $object2->brigadista    = $inputs['brigadista_'.$i];
                $object2->vacunador     = $inputs['vacunacion_'.$i];
                $object2->dengue        = $inputs['dengue_'.$i];
                $object2->save();
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $obj_detalle    = RelSubbrigadistaMes::where("subbrigadista_id", $id)->delete();
        $obj            = SubBrigadista::find($id);
