...
class TrabajadorController extends Controller
{
    ...
    private function aplicarFiltros($main_query, $parametros, $access){
        ...
        if(isset($parametros['active_filter']) && $parametros['active_filter']){
            ...
            if(isset($parametros['opd']) && $parametros['opd'] == 2){
                $main_query = $main_query->whereRaw("trabajador.id in (select trabajador_id from rel_trabajador_datos_laborales_nomina  where (basificados=0 and ur!='420_OPD'))");
            }
    ...
    public function show(Request $request, $id)
    {
        try{
            ...
            $trabajador = Trabajador::with(['municipio_nacimiento','datoslaborales','escolaridad','horario',
            'datoslaboralesnomina' => fn($q) => $q->withTrashed(), 'datoslaboralesnomina.clues', 'datoslaboralesnomina.cr', 'credencial',"rel_datos_fiscales"])->where("id", "=", $id);
