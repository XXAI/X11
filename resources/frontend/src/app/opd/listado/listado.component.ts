      ...
      response =>{
        if(response.error) {
          this.error_pdf(response);
            ...
            const reportWorker = this.iniciateWorker('ANEXOS_'+response.data.clues.replace(" ","_"));
      ...
      response =>{
        if(response.error) {
          this.error_pdf(response);
            ...
            const reportWorker = this.iniciateWorker('ANEXO_3_'+response.data.clues.replace(" ","_"));
            let config = {  title: this.reportTitle, lote:false };
